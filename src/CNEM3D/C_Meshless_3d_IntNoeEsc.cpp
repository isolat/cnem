...
int C_Meshless_3d::Calcul_Interpolation_Noeud_Esclave(vector<long>* P_List_Ind_Noeud_FoFo, vector<double>* P_List_FoFo,
	vector<long>* P_List_Ind_Noeud_Esclave_Front, vector<long>* P_List_Ind_Noeud_Esclave_Internal)
{
	bool del_0 = FALSE;
	bool del_1 = FALSE;
	if (P_List_Ind_Noeud_Esclave_Front == NULL) { P_List_Ind_Noeud_Esclave_Front = new vector<long>; del_0 = TRUE; }
	if (P_List_Ind_Noeud_Esclave_Internal == NULL) { P_List_Ind_Noeud_Esclave_Internal = new vector<long>; del_1 = TRUE;}

    long i;
	for (i = Nb_Noeud_Ini; i < Diag_Vor.Nb_Noeud; i++)
	{
		if(Tab_Ind_Noeud_Front[i])P_List_Ind_Noeud_Esclave_Front->push_back(i);
		else P_List_Ind_Noeud_Esclave_Internal->push_back(i);
	}

	if (del_0 && del_1)
	{
		if (P_List_Ind_Noeud_Esclave_Internal->size() != 0)
		{
			printf("\n%i internal steiner point !\n", P_List_Ind_Noeud_Esclave_Internal->size());
			delete P_List_Ind_Noeud_Esclave_Front;
			delete P_List_Ind_Noeud_Esclave_Internal;
			return 1;
		}
	}

    Calcul_Interpolation_Noeud_Esclave_Front(P_List_Ind_Noeud_Esclave_Front,P_List_Ind_Noeud_FoFo,P_List_FoFo);
	Calcul_Interpolation_Noeud_Esclave_Internal(P_List_Ind_Noeud_Esclave_Internal, P_List_Ind_Noeud_FoFo, P_List_FoFo);

	if (del_0) delete P_List_Ind_Noeud_Esclave_Front;
	if (del_1) delete P_List_Ind_Noeud_Esclave_Internal;
