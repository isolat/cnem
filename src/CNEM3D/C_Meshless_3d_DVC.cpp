...
int Appel_Tetgen_Exe(void* args_list)
{
    ...
    C_Meshless_3d* PML=(C_Meshless_3d*)Args_List[0];
    char* Arg_Tet=(char*)Args_List[1];
	char* bin_path = (char*)Args_List[2];
    
    //-----------------------------------------------------------------------//

    string Tetgen_Path,Tetgen_Exe_Name;
	Tetgen_Path = (bin_path == NULL) ? "" : bin_path;
#ifdef _WIN32
...
bool C_Meshless_3d::Voronoi_Contrain_TetGen(int Type_Appel,char* Arg_Tet,long ind_group_tet,bool erase_tet_out,char* bin_path)
{
    ...
    void* Args_List[3]={(void*)this,(void*)Arg_Tet,(void*)bin_path};
...
bool C_Meshless_3d::Voronoi_non_Contrain_from_tet(vector<int>* p_trifacelist, vector<int>* p_tetrahedronlist, vector<int>* p_neighborlist)
{
	cout << "\nChargement Tetraedrisation Delaunay -------------------------------------------\n" << endl;

	//-----------------------------------------------------------------------//

	Ind_Group_Tet = 0;
	Erase_Tet_Out = 0;
	if (Nb_Tri_Front != 0) return 0;

	//--------------------------------------------------------------------------//

	vector<int> in_pointmarkerlist(Diag_Vor.Nb_Noeud);
	long i; for (i = 0; i < Diag_Vor.Nb_Noeud; i++)in_pointmarkerlist[i] = i + 1;

	//--------------------------------------------------------------------------//

	int Retour_Load_TDC = Load_Tetgen_TDC(Diag_Vor.Nb_Noeud, Diag_Vor.Nb_Noeud, p_trifacelist->size()/3, p_tetrahedronlist->size()/4,
		in_pointmarkerlist.data(),Diag_Vor.Tab_Noeud, p_trifacelist->data(), NULL, p_tetrahedronlist->data(), p_neighborlist->data());
