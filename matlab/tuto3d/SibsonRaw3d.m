...
%% compute raw Sibson shape function

Interpol=m_cnem3d_interpol(XYZ_Noeud,[],XYZ_Point,3);


%% check that grad node is [1,0,0;0,1,0;0,0,1]

if size(Interpol.Mat_INT_NN,1)~=0 % if there is new node
    XYZ_Noeud=[XYZ_Noeud;Interpol.Mat_INT_NN*XYZ_Noeud];
end 

grad_x_node=Interpol.Mat_GX*XYZ_Noeud;
grad_y_node=Interpol.Mat_GY*XYZ_Noeud;
grad_z_node=Interpol.Mat_GZ*XYZ_Noeud;

nb_Pnt=size(XYZ_Point,1);

j=0;
for i=1:nb_Pnt
    if Interpol.Nb_V(i)>3 
        % ie :
